document.addEventListener("DOMContentLoaded", function () {
    ...
    const schedulerSection1 = new AnimationScheduler();
    ...
    function changeSVGColor(object, color) {
      ...
      elements.forEach(el => {
        setTimeout(() => {
          el.setAttribute('fill', color); // 变更颜色
        }, 10);
      });
    }
    ...
    iconObjects.forEach((object, index) => {
      ...
      changeSVGColor(object, white);
      setTimeout(() => {
        schedulerSection1.addAnimation(() => changeSVGColor(object, "red"), delayAfterAOS);
        if (index === iconObjects.length - 1) schedulerSection1.run(); // Start scheduler on the last item
      }, aosDuration);
    });
