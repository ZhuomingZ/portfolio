    ...
    function changeObjectSvgPathColor({ selector, startColor, endColor, duration = 1000 }) {
      ...
      iconObjects.forEach(object => {
        object.onload = function () {
          ...
          paths.forEach(path => {
            path.setAttribute('fill', startColor); // 初始颜色
            path.style.transition = `fill ${duration}ms ease-in-out`;
            path.setAttribute('stroke', startColor); // 初始颜色
            path.style.transition = `stroke ${duration}ms ease-in-out`;
    
            setTimeout(() => {
              path.setAttribute('fill', endColor); // 过渡到目标颜色
              path.setAttribute('stroke', endColor); // 过渡到目标颜色
            }, 10); // 确保动画生效
          });
        };
      });
